...
local update_rate = settings.global["update-rate"].value * 60

-- GUI
local function add_quality_status(event)
	if settings.get_player_settings(event.player_index)["show-bonus-quality-gui"].value == false then
		game.print("Quality GUI is disabled.")
		return
	end
...
script.on_event(defines.events.on_built_entity, add_quality_record, {
	{ filter="crafting-machine" }, --TODO: drills and fuel output
})
